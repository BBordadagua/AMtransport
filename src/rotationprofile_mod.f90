Module rotationprofile_mod
  ...
  subroutine rotation_profile_CN_radiative(MESAold,omega,model,m)
    implicit none
    real (DP) :: AMi,AMf, delta_r, delta_t,nu_diff
    real (DP), intent(inout) :: omega(:),MESAold(:,:)
    real (DP), allocatable :: MESAfile(:,:),MESAold_int(:,:),MESA_unigrid(:,:)
    real (DP), allocatable ::  omega_rad(:), radius(:),omega_old(:)
    ...
    !! create uniform grid
    call getradiativeR(ri,rf,MESAfile)
    m_nonuni = rf-100 !! how many points before convective boundary
  ...
  iter = 100
  delta_t = 0.01*dt
  omega_rad = CrankNicolson(MESA_unigrid,mrad,iter,delta_t,nu_diff)
  ...
  do i=2,m
    ...
    deallocate(omega_old,radius)
    deallocate(MESAfile,MESAold_int,MESA_unigrid)
